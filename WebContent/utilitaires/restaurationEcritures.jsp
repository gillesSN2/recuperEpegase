...
<f:subview id="restecrdet">

    <a4j:form>

        <center> <h2><h:outputText value="RESTAURATION ECRITURES DETRUITES" style="color:green;"/></h2></center>
        ...
        <h:panelGrid  columns="1" width="100%">
            <h:panelGrid id="panCtrl"  styleClass="recherche" width="100%">
                <h:panelGrid columns="6" width="100%">
                    ...
                    <h:column><h:outputText style="text-decoration:underline;" value="Journal:"/></h:column>
                    <h:column>
                        <h:selectOneMenu style="width:211;" value="#{bakingbeanepegase.menuModuleHorizontalCtrl.formBakingBeanAdministration.formUtilitaires.var_journal}">
                            <f:selectItem itemLabel="Sélection journal" itemValue=""/>
                            <f:selectItems value="#{bakingbeanepegase.menuModuleHorizontalCtrl.formBakingBeanAdministration.formUtilitaires.mesJournauxComptables}"/>
                        </h:selectOneMenu>
                    </h:column>
                </h:panelGrid>
            </h:panelGrid>
            <h:panelGrid id="select" columns="3" width="100%" columnClasses="clos30g,clos30g,clos30g">
                <a4j:commandButton value="Rechercher Ecritures" title="Recherche les écritures détruites" action="#{bakingbeanepegase.menuModuleHorizontalCtrl.formBakingBeanAdministration.formUtilitaires.rechercheEcrituresDetuites}" style="width:100%" onclick="javascript:Richfaces.showModalPanel('modAttente');" oncomplete="javascript:Richfaces.hideModalPanel('modAttente');" reRender="modAttente,tableExtrait,scrollTable"/>
                <a4j:commandButton value="Tout sélec." title="Sélectionne toutes les écritures de la liste" reRender="tableExtrait,scrollTable" action="#{bakingbeanepegase.menuModuleHorizontalCtrl.formBakingBeanAdministration.formUtilitaires.toutSelectionner}" style="width:100%"/>
                <a4j:commandButton value="Rien sélec." title="Dé-sélectionne toutes les écritures de la liste" reRender="tableExtrait,scrollTable" action="#{bakingbeanepegase.menuModuleHorizontalCtrl.formBakingBeanAdministration.formUtilitaires.toutDeSelectionner}" style="width:100%"/>
            </h:panelGrid>
            <h:panelGrid width="100%">
                <a4j:region renderRegionOnly="false">
                    ...
                    <rich:extendedDataTable rows="300" border="0" width="100%" noDataLabel=" " styleClass="bg" headerClass="headerTab" style="max-height:100%;border:solid 0px green;" id="tableExtrait" selectedClass="active-row" value="#{bakingbeanepegase.menuModuleHorizontalCtrl.formBakingBeanAdministration.formUtilitaires.dataModelEcrituresDetruites}" var="table" rowClasses="rows1,rows2,rowsd">
                        <a4j:support eventsQueue="maQueue" event="onRowClick" action="#{bakingbeanepegase.menuModuleHorizontalCtrl.formBakingBeanAdministration.formUtilitaires.selectionEcrituresDetuites}"/>
                        ...
                        <rich:column id="c5" width="9%" sortable="true" sortBy="#{table.ecrReference1}" label="Référence 1">
                            <f:facet name="header"><h:outputText  value="Réf.1"/></f:facet>
                            <h:outputText value="#{table.ecrReference1}">
                            </h:outputText>
                        </rich:column>
                        <rich:column id="c6" width="6%" sortable="true" sortBy="#{table.ecrReference2}" label="Référence 2">
                        ...
                        <rich:column id="c12" width="26%" sortable="true" sortBy="#{table.ecrLibelle}" label="Libellé">
        ...
        <h:panelGroup id="panelBoutonTrf">
            <center>
                ...
                <h:commandButton id="idValide" action="#{bakingbeanepegase.menuModuleHorizontalCtrl.formBakingBeanAdministration.formUtilitaires.restaurationEcritures}" value="Restauration écritures détruites" style="color:white;background-color:green;margin:3px 3px 3px 3px;width:200px;" onclick="if (!confirm('Etes-vous sur de vouloir exécuter cette opération ?')) return false;javascript:Richfaces.showModalPanel('panelBarProg');"/>
            </center>
        </h:panelGroup>
