...
public class LectureDevisDelaisLivraison implements Serializable {
   private ObjetCompte objetCompte;
   private List mesElements = new ArrayList();
   private List mesElementsItems;

   public LectureDevisDelaisLivraison(long var1) {
      this.recupereDelaisLivraison(var1);
   }

   public void recupereDelaisLivraison(long var1) {
      try {
         SAXBuilder var3 = new SAXBuilder();
         boolean var4 = false;
         File var5 = new File(StaticModePegase.getCheminContext() + File.separator + "clients" + File.separator + "structure" + var1 + File.separator + "ventes" + File.separator + "configuration" + File.separator + "devisDelaisLivraison.xml");
         if (var5.exists()) {
            var4 = true;
         }
