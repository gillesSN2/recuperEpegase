...
public class LectureInfirmerieAudiometrie implements Serializable {
   private ObjetCompte objetCompte;
   private List mesElements = new ArrayList();
   private List mesElementsItems;

   public LectureInfirmerieAudiometrie() {
      this.recupereVaccin();
   }
   ...
   public void recupereVaccin() {
      try {
         ...
         File var2 = new File(StaticModePegase.getCheminContext() + File.separator + "configuration" + File.separator + "commercial" + File.separator + "medicalAudiometrie.xml");
